...
- [x] `.github/workflows/scraper-schedule.yml` を更新
  - コミット対象を `data/shogi.db` → `src/frontend/data/tournaments.json` に変更
- [x] `src/frontend/app.js` を書き換え
...
- [x] `render.json` を Static Site 用に変更
- [x] 削除対象:
  - `server.py`
  - `src/api/` ディレクトリ
  ...
  - `data/shogi.db`
  - 関連テスト
- [x] `pyproject.toml` から不要な依存を整理（`fastapi`, `uvicorn` 等）
- [x] 初期 `tournaments.json` を生成してコミット

### Phase B: 人間が実施すること

- [x] Render ダッシュボードで Web Service → Static Site に切り替え（または旧サービス削除 → 新規 Static Site 作成）
  - Publish directory: `src/frontend`
- [x] 本番 URL でアプリが表示されることを確認
- [x] フィルタ・地図が正常に動作することを目視確認
